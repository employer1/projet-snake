...
(() => {
    ...
    const chargerStatistiques = async () => {
        ...
        try {
            const reponse = await fetch(`../../module/dactylo/${fichierStatistiques}`, { cache: "no-store" });
            if (!reponse.ok) {
    ...
    const terminerTest = async () => {
        ...
        window.location.href = "dactylo_resultats.html";
    };
    ...
    const chargerMots = async () => {
        ...
        if (!texte) {
            const reponse = await fetch("../../module/dactylo/top_1000.txt");
            if (!reponse.ok) {
