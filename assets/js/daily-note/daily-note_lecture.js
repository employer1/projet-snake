...
let timerConfirmationSauvegarde = null;
let titreAvantConfirmationSauvegarde = "";
...
const afficherConfirmationSauvegarde = () => {
    const titrePage = document.getElementById("daily-note-lecture-title");
    if (!titrePage) {
        return;
    }

    if (timerConfirmationSauvegarde) {
        clearTimeout(timerConfirmationSauvegarde);
    } else {
        titreAvantConfirmationSauvegarde = titrePage.textContent;
    }

    titrePage.textContent = "Sauvegardé";
    timerConfirmationSauvegarde = setTimeout(() => {
        titrePage.textContent = titreAvantConfirmationSauvegarde;
        titreAvantConfirmationSauvegarde = "";
        timerConfirmationSauvegarde = null;
    }, 2000);
};
